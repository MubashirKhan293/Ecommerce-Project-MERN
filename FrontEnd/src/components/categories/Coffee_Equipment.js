...
function CoffeeEquipment() {
  const navigate = useNavigate();
  const location = useLocation(); // Get the current location
  const [data, setData] = useState([]);
  const [activeCategory, setActiveCategory] = useState('/coffee-equipment'); // Initialize active category
  ...
  const coffeeEquipment = data.filter(item => item.category === "Coffee Equipment");
